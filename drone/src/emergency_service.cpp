...
public:
    ImageConverter()
            : it_(nh_)
    {
        image_sub_=it_.subscribe("/ardrone/bottom/image_raw",1,&ImageConverter::imageCb,this); //subscribe to webcam image
...
//the color in this tutorial is bright yellow (tennis ball), the color range should be adjusted according to your object
        cv::inRange(HSVImage,cv::Scalar(5, 50, 50),cv::Scalar(15, 255, 255),mask); //detecting colors
